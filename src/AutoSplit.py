...
class AutoSplit(QMainWindow, design.Ui_MainWindow):
    ...
    hl_chapter_select = True
    hl_broken_bridge = True
    hl_pink_desert = True
    hl_sunken_city = True
    hl_underground = True
    hl_tower = True
    ...
    # Number of false loading screen during every level HL  (screens that are the same as the expected loading screen)
    fake_load_chapter_select = 4
    fake_load_broken_bridge = 3
    fake_load_pink_desert = 3
    fake_load_sunken_city = 0
    fake_load_underground = 0
    fake_load_tower = 5
    fake_load_snow = 1
    fake_load_paradise = 0

    # Does the runner uses the zen jump trick in sunken city (involves opening the menu -> fake loading screen)
    sunken_city_zen_jump = True

    if sunken_city_zen_jump == True : 
        fake_load_sunken_city = fake_load_sunken_city + 1

    fake_loads_array = [
        fake_load_chapter_select, 
        fake_load_broken_bridge, 
        fake_load_pink_desert, 
        fake_load_sunken_city, 
        fake_load_underground, 
        fake_load_tower, 
        fake_load_snow,
        fake_load_paradise
    ]
    ...
    # Current level 
    current_level = Level.CHAPTER_SELECT


    # Fake loading screen counter
    fake_loading_counter = 0
    ...
    def browse(self):
        ...
        self.split_image_directory = str(
            QFileDialog.getExistingDirectory(self, "Select Split Image Directory")) + '\\'
    ...
    def alignRegion(self):
        ...
        template_filename = str(QFileDialog.getOpenFileName(self, "Select Reference Image", "",
    ...
    def autoSplitter(self):
        ...
        self.hl_array = [
            self.hl_chapter_select,
            self.hl_broken_bridge,
            self.hl_pink_desert,
            self.hl_sunken_city,
            self.hl_underground,
            self.hl_tower,
        ...
        # Number of false loading screen during every level HL  (screens that are the same as the expected loading screen)
        self.fake_load_chapter_select = 4
        self.fake_load_broken_bridge = 3
        self.fake_load_pink_desert = 3
        self.fake_load_sunken_city = 0
        self.fake_load_underground = 0
        self.fake_load_tower = 5
        self.fake_load_snow = 1
        self.fake_load_paradise = 0

        # Does the runner uses the zen jump trick in sunken city (involves opening the menu -> fake loading screen)
        self.sunken_city_zen_jump = self.sunken_city_zen_jump_checkbox.isChecked()

        if self.sunken_city_zen_jump == True : 
            self.fake_load_sunken_city = self.fake_load_sunken_city + 1

        self.fake_loads_array = [
            self.fake_load_chapter_select, 
            self.fake_load_broken_bridge, 
            self.fake_load_pink_desert, 
            self.fake_load_sunken_city, 
            self.fake_load_underground, 
            self.fake_load_tower, 
            self.fake_load_snow,
            self.fake_load_paradise
        ]
        ...
        while self.split_image_number < self.number_of_split_images:
            ...
            while True : 
                ...
                # calculate similarity for split image
                self.similarity = self.compareImage(self.split_image, self.mask, capture)
                # print("getting similarity : " + str(self.similarity))
            ...
            # if we detect this screen is a fake loading screen 
            if self.fake_loading_counter < self.fake_loads_array[self.current_level] and self.hl_array[int(self.current_level)]== True : 
                # increase the fake loading counter
            ...
            if self.number_of_split_images != self.split_image_number:
                ...
                # This should probably eventually be a signal... but it works
                pause_start_time = time.time()
                while time.time() - pause_start_time < self.pause_time:
                    # check for reset
                    if win32gui.GetWindowText(self.hwnd) == '':
                        self.reset()
                    if self.startautosplitterButton.text() == 'Start Auto Splitter':
    ...
    def loadSettings(self):
        try:
            ...
            # Forcing similarity threshold to 95
            # self.similaritythresholdDoubleSpinBox.setValue(95)
            ...
            # Setting to histogram method
            # self.comparisonmethodComboBox.setCurrentIndex(1)
...
class SelectWindowWidget(QWidget):
    def __init__(self):
...
# About Window
class AboutWidget(QWidget, about.Ui_aboutAutoSplitWidget):
    def __init__(self):
